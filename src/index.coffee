...
class Bot
    @create = (options = {}) ->
        ...
        ENV.prNumber = if process.env['CIRCLE_PULL_REQUEST'] then basename(process.env['CIRCLE_PULL_REQUEST']) else (if process.env['CI_PULL_REQUEST'] then basename(process.env['CI_PULL_REQUEST']) else '')
